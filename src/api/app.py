...
def full_mock() -> dict:
    """Return the 257-pick full 7-round mock generated by scripts/build_full_mock.py.
    Each pick: {pick, round, team, player, position, college, rank, tier,
    reasoning, factors, alternates}. Empty picks array if not yet built."""
    if not FULL_MOCK_JSON.exists():
        return {"generated_at": None, "n_picks": 0, "picks": [], "methodology": None}
    return json.loads(FULL_MOCK_JSON.read_text(encoding="utf-8"))
